...
class EditMedicineActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        with(binding) {
            medicine?.let { dataMedicine ->
                with(cardMedicine) {
                    Glide.with(image.context)
                        .load(
                            when (dataMedicine.type) {
                        ...
                        .into(image)
                    name.text = String.format("%dx %s", dataMedicine.amount, dataMedicine.name)
                    description.text = dataMedicine.description
                    time.text = dataMedicine.dateTime.format(timeFormat)
                }

                inputName.editText?.setText(dataMedicine.name)
                inputType.editText?.setText(dataMedicine.type)
                inputDescription.editText?.setText(dataMedicine.description)
                inputDate.editText?.setText(dataMedicine.dateTime.format(DateTimeFormatter.ofPattern("EEEE, dd MMMM yyyy", Locale.getDefault())))
                inputTime.editText?.setText(dataMedicine.dateTime.format(timeFormat))
                inputAmount.editText?.setText(dataMedicine.amount.toString())
            }
                ...
                if (isNameFilled and isTypeFilled and isDescriptionFilled and isDateFilled and isTimeFilled and isAmountFilled) {
                    medicine?.let { dataMedicine ->
                        viewModel.editMedicine(
                            context = this@EditMedicineActivity,
        ...
        binding.btnDelete.setOnClickListener {
            closeKeyboard()
            medicine?.let { dataMedicine ->
                viewModel.deleteMedicine(this@EditMedicineActivity, dataMedicine)
            }
            onBackPressed()
        }
